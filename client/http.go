...
const (
	...
	getTestsTimesEndpoint = "/tests/timedata?accountId=%s&orgId=%s&projectId=%s&pipelineId=%s&buildId=%s&stageId=%s&stepId=%s&parentUniqueId=%s&buildStartTime=%d&enableAverages=%t"
...
func (c *HTTPClient) UploadCgV2JSON(ctx context.Context, jsonPayload []byte, stepID string, timeMs int64, sourceBranch string, targetBranch string) error {
	...
	_, err := c.retryWithOptions(ctx, c.Endpoint+path, "POST", "", json.RawMessage(jsonPayload), nil, false, true, backoff, true) //nolint:bodyclose
	return err
}
...
func (c *HTTPClient) GetTestTimes(ctx context.Context, stepID string, in *types.GetTestTimesReq, buildStartTime int64, enableAverages bool) (types.GetTestTimesResp, error) {
	var resp types.GetTestTimesResp
	if err := c.validateGetTestTimesArgs(); err != nil {
		return resp, err
	}
	path := fmt.Sprintf(getTestsTimesEndpoint, c.AccountID, c.OrgID, c.ProjectID, c.PipelineID, c.BuildID, c.StageID, stepID, c.ParentUniqueID, buildStartTime, enableAverages)
	backoff := createBackoff(10 * 60 * time.Second)
...
func (c *HTTPClient) GetSkipTests(ctx context.Context, req v2types.SkipTestsRequest) (types.SkipTestResponse, error) {
	var resp types.SkipTestResponse
	if err := c.validateSubmitChecksumsArgs(req.Files); err != nil {
		return resp, err
	}
