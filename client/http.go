...
const (
	...
	getTestsTimesEndpoint = "/tests/timedata?accountId=%s&orgId=%s&projectId=%s&pipelineId=%s&buildId=%s&stageId=%s&stepId=%s&buildStartTime=%s"
...
func (c *HTTPClient) GetTestTimes(ctx context.Context, stepID string, in *types.GetTestTimesReq, buildStartTime string) (types.GetTestTimesResp, error) {
	...
	path := fmt.Sprintf(getTestsTimesEndpoint, c.AccountID, c.OrgID, c.ProjectID, c.PipelineID, c.BuildID, c.StageID, stepID, buildStartTime)
	backoff := createBackoff(10 * 60 * time.Second)
